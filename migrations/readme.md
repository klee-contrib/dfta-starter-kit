...
Les mises à jour de la base de données sont gérées par Entity Framework, à l'aide de l'outil `dotnet-ef`. Installez-le avec la commande `dotnet tool install --global dotnet-ef`. Sa documentation est [disponible ici](https://docs.microsoft.com/en-us/ef/core/cli/dotnet).
...
Pour créer une migration, exécutez la commande `dotnet ef migrations add XXXXX` depuis le dossier courant. Elle est générée à partir de l'état du modèle après la dernière migration créée (s'il ne s'agit pas de la première). Cet état est conservé dans `Migrations/KleeContribDftaDbContextModelSnapshot.cs`.

Pour appliquer les migrations, lancez le projet de migrations avec `dotnet run` ou depuis Visual Studio. La configuration de la base de données suit le même principe que celle de l'application Web : les paramètres communs sont définis dans `appsettings.json`, et les autres sont fournis sous forme de variables d'environnement, notamment via `launchSettings.json`.
...
- `true` : supprimer puis recréer la base de données avant toute autre opération. Utilisez cette option en local ou via la pipeline dédiée "Reset DB".
...
Vous pouvez également utiliser l'option `--check` pour effectuer uniquement la vérification.

_Remarque : n'utilisez pas la commande `dotnet ef database update` pour appliquer les migrations, contrairement à ce qui peut être indiqué dans la documentation de la CLI EF._
...
Faites attention à bien conserver la migration initiale `CreateDb` : elle configure notamment le compte de base de données utilisé par l'application.
