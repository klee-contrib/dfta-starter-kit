...
public class ProfilMutations(KleeContribDftaDbContext context) : IProfilMutations
{
    ...
    public async Task<int> AddProfil(ProfilCommand profil, CancellationToken ct = default)
    {
        ...
        context.DroitProfils.AddRange((profil.DroitCodes ?? []).Select(droitCode => new DroitProfil
        {
            DroitCode = droitCode,
    ...
    {
        var profilDb = await context.Profils.FindAsync([proId], ct) ?? throw new KeyNotFoundException("Le profil demandé n'existe pas.");
        ...
        context.DroitProfils.AddRange((profil.DroitCodes ?? []).Select(x => new DroitProfil
        {
            DroitCode = x,
            ProfilId = profilDb.Id
        }));
